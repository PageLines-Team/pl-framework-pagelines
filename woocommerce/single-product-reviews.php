	...
	<!--
		...
		<?php if ( have_comments() ) { ?>
			...
			<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ){
				...
				?> <p class="woocommerce-noreviews"><?php _e( 'There are no reviews yet.', 'woocommerce' ); ?></p> <?php
		}
	echo '</div>';

		// check user has not commented before...
		...
		$comments_array = $wpdb->get_results($query, OBJECT);

		if( empty( $comments_array ) ) : // has not reviewed before...
		?>
		...
		<?php else : ?>
			<p class="woocommerce-verification-required"><?php _e( 'Looks like you have already left a review for this product.', 'woocommerce' ); ?></p>
			<?php endif; ?>
