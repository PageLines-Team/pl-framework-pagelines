  ...
  <div id="comment-<?php comment_ID(); ?>" class="comment_container">
    ...
    <div class="comment-text">
      ...
      <?php if ( $comment->comment_approved == '0' ) : ?>
        ...
        <p class="meta"><em><?php _e( 'Your review is awaiting approval', 'woocommerce' ); ?></em></p>

      <?php else : ?>

        <?php $url = get_the_author_meta( 'user_url', $comment->user_id );

        if( '' != $url ) {
          $author_link = sprintf( '<a href="%s">%s</a>', $url, get_comment_author() );
        } else {
        ...
        ?>
        <p class="meta">
          <strong itemprop="author"><?php echo $author_link; ?></strong> <?php
